...
st.title("Example: Papers with at least one Author affiliated with Xian Institute of Optics and Precision Mechanics")
st.markdown("[XIOPM](https://english.opt.cas.cn/)")
st.write("Topic modeling")

@st.cache_data()
def load_centroids_xian():
    #dg = pd.read_csv("penguins.csv", engine="pyarrow")
  #  df = pd.read_json(df.to_json())
    dg = pd.read_pickle('xiancentroids2d.pkl.gz')
    return dg[dg.cluster != -1]

@st.cache_data()
def load_dftriple_xian():
    dg = pd.read_pickle('xiandftriple2d.pkl.gz')
    return dg

@st.cache_data()
def load_dfinfo_xian():
    dg = pd.read_pickle('xiandfinfo2d.pkl.gz')
    return dg[dg['cluster'] != -1]



centroids = load_centroids_xian()
dftriple = load_dftriple_xian()
dfinfo = load_dfinfo_xian()
