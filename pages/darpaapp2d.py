...
st.title("Example: Finding Papers Similar to DARPA-Funded Research")
...
st.markdown("""Computer Science, Physics, Materials Science,
     Quantum mechanics, Mathematics, Artificial intelligence, Biology,
     Engineering, Optics, Chemistry, Optoelectronics, Medecine, Gene,
     Nanotechnology, Composite material, Programming language, Biochemistry,
     Genetics, Geometry, Telecommunications, Algorithm, Operating System,
     Machine Learning, Statistics, Laser
    """
)
...
def get_fig_darpa():
    ...
    fig_papers = px.scatter(dfinfo[dfinfo.cluster != -1],
                           x='x',y='y',
                    color='darpa_funded',
                        hover_data = ['title','cluster','darpa_funded',
                                      'funders',
                                      'publication_date'])
...
st.write("Use the navigation tools in the mode bar to pan and zoom. Papers are automatically clustered into subtopics. Topics are the bigger pink dots with representative keywords and phrases available on hover. Using the box or lasso select then returns a tabular list of those selected papers. Papers close to the blue dots (DAPRA-funded) can be selected.")

# https://www.youtube.com/watch?v=htXgwEXwmNs
selected_points = plotly_events(bigfig, select_event=True,
                               click_event=False,override_height=700)
if len(selected_points) == 0:
    st.stop()
    
#st.write(selected_point)
# need to figure out a way to deal with the case of lassoing on a centroid topic point

#st.write(selected_points)

selected_x_values = [selected_points[i]["x"] for i in range(len(selected_points))]
selected_y_values = [selected_points[i]["y"] for i in range(len(selected_points))] 
#selected_species = selected_point[0]["species"]
#st.write(selected_x_values)
#st.write(selected_y_values)
#st.write(type(selected_x_values))
#huh = [54,56,67]
#st.write(huh)
#df_selected = pd.DataFrame({"x": selected_x_values,
#                         "y": selected_y_values}).merge(dfinfo)
#try:

def get_mini_frame(i):
    dfmini = dfinfo[
        (dfinfo["x"] == selected_x_values[i])
    & (dfinfo["y"] == selected_y_values[i])
    ].copy()
    return dfmini
...
for i in range(len(selected_x_values)):
    dfmini = get_mini_frame(i)
    frames_list.append(dfmini)
    
df_selected = pd.concat(frames_list, ignore_index=True)
df_selected.sort_values('darpa_funded',ascending=False, inplace=True)


#df_selected = dfinfo[
#        (dfinfo["x"] == selected_x_values[0])
#    & (dfinfo["y"] == selected_y_values[0])
#    ].copy()
...
st.data_editor(
       ...
       'publication_date', 'keywords', 'top_concepts', 'affil_list','funders',
       'author_list','darpa_funded']],
        column_config={
